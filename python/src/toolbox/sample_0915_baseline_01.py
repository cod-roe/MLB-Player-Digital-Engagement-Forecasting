...
def train_lgb(
    ...
    for nfold in list_nfold:
        ...
        if not os.path.isfile(os.path.join(EXP_MODEL, f'model_lgb_fold{nfold}.pickle')): # if trained model, no training
            #train
...
# utils
# ログファイルの設定
logging.basicConfig(filename=f'{OUTPUT_EXP}/log_{name}.txt', level=logging.INFO, format='%(message)s')
# ロガーの作成
...
df_submit = pd.DataFrame({'PassengerId': id_test['PassengerId'], 'Survived': pred}).set_index('PassengerId')
